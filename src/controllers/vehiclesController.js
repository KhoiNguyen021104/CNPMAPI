import { StatusCodes } from 'http-status-codes'
import { vehiclesService } from '~/services/vehiclesService'


const create = async (req, res, next) => {
  try {
    const resCreated = await vehiclesService.create(req.body)
    res.status(StatusCodes.CREATED).json(resCreated)
  } catch (error) {
    next(error)
  }
}

const getAll = async (req, res, next) => {
  try {
    const response = await vehiclesService.getAll()
    res.status(StatusCodes.OK).json(response)
  } catch (error) {
    next(error)
  }
}

const findOneById = async (req, res, next) => {
  try {
    const vehicle = await vehiclesService.findOneById(req.params._id)
    if (!vehicle) res.status(StatusCodes.NOT_FOUND).json({ message: 'Vehicle not found' })
    else res.status(StatusCodes.OK).json(vehicle)
  } catch (error) {
    next(error)
  }
}

const update = async (req, res, next) => {
  try {
    const resUpdated = await vehiclesService.update(req.params._id, req.body)
    if (!resUpdated) res.status(StatusCodes.NOT_FOUND).json({ message: 'Vehicle not found' })
    else res.status(StatusCodes.OK).json(resUpdated)
  } catch (error) { next(error) }
}

const deleteVehicle = async (req, res, next) => {
  try {
    await vehiclesService.deleteVehicle(req.params._id)
    res.status(StatusCodes.NO_CONTENT).send()
  } catch (error) {
    next(error)
  }
}


export const vehiclesController = {
  create,
  getAll,
  findOneById,
  update,
  deleteVehicle
}